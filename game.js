import { PAIRS, LILAS_PAIRS, DIFFICULTIES, BACK_LOGO, TIPS } from './data.js';
...
const LS_PLAYER_ID = 'cuidese-player-id';
...
function activePairs() {
  const count = S.config.lilas || 0;
  const lilas = LILAS_PAIRS.slice(0, count);
  const regular = PAIRS.slice(0, S.config.pairs - count);
  return [...regular, ...lilas];
}
...
function makeCard(pair, side) {
  ...
  const back = pair.back
    ? `<div class="back-photo"><img src="${pair.back}" alt="Agosto Lilás"></div>`
    : `<div class="back-badge">${BACK_LOGO}</div>`;
...
document.getElementById('startLeaderboardBtn')
  .addEventListener('click', () => {
    void showLeaderboard();
  });
...
victoryNameInput.addEventListener('keydown', e => {
  if (e.key === 'Enter') {
    void saveScore();
  }
});
